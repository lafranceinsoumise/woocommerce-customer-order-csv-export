...
class WC_Customer_Order_CSV_Export_Generator {
	...
	private function get_orders_csv_row_data( $order_id ) {
		...
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			if ( $is_json ) {
				...
				foreach ( $item_meta->get_formatted( '_' ) as $meta_key => $formatted_meta ) {
					...
					$meta[ $formatted_meta['key'] ] = str_replace( '"', '&quot;', $formatted_meta['value'] );
				}
			...
			// tax data is only supported in JSON-based formats, as encoding/esacping it reliably in
		...
		foreach ( $order->get_refunds() as $refund ) {
			...
			// line items data for refunds is only supported in JSON-based formats, as encoding/esacping it reliably in
			// a pipe-delimited format is quite messy
			if ( $is_json ) {
				...
				foreach ( $refund->get_items( array( 'line_item', 'fee', 'shipping' ) ) as $item_id => $item ) {
					...
					if ( $is_json ) {
						...
						if ( isset( $item['taxes'] ) ) {
							...
							// line & fee items use `line_tax_data`, with both total and subtotal tax detals
		...
		$order_data = array(
			...
			'billing_state'          => $this->get_localized_state( $order->billing_country, $order->billing_state ),
			'billing_state_code'     => $order->billing_state,
			...
			'shipping_state'         => $this->get_localized_state( $order->shipping_country, $order->shipping_state ),
	...
	private function get_customers_csv_row_data( $id, $order_id = null ) {
		...
		if ( ! $user && is_numeric( $order_id ) ) {
			...
			if ( $order ) {
				...
				$user->billing_state       = $this->get_localized_state( $order->billing_country, $order->billing_state );
				$user->billing_state_code  = $order->billing_state;
				...
				$user->shipping_state      = $this->get_localized_state( $order->shipping_country, $order->shipping_state );
				$user->shipping_state_code = $order->shipping_state;
	...
	/**
	 * Backwards compat for renaming the $order_format member.
	 *
	 * @TODO Remove this as part of WC 2.7 compat {TZ 2016-08-09}
	 *
	 * @since 4.0.2
	 * @param string $name
	 */
	public function __get( $name ) {
		...
		switch ( $name ) {
			...
			case 'order_format':
				...
				_deprecated_function( 'WC_Customer_Order_CSV_Export_Generator::order_format', '4.0.2', 'WC_Customer_Order_CSV_Export_Generator::export_format' );
